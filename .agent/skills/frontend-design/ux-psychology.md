...
> The most commonly violated laws in AI-generated UIs. Check these first.

| Law                   | Most Common Violation                   | Quick Fix                                                 |
| --------------------- | --------------------------------------- | --------------------------------------------------------- |
| **Hick's Law**        | Navigation with 8–12 items              | Collapse to 5 max + "More"                                |
| **Fitts' Law**        | CTA buttons under 36px tall             | `h-11` (44px) minimum, `h-12` on mobile                   |
| **Jakob's Law**       | Logo doesn't link to home               | Always `<Link href="/">`                                  |
| **Doherty Threshold** | Button click with no immediate feedback | `disabled` + spinner state within 100ms                   |
| **Von Restorff**      | Primary and secondary CTAs same color   | Only one button gets the accent color                     |
| **Gestalt Proximity** | Label and input far apart               | `gap-1.5` between label and field, `gap-6` between fields |
| **Peak-End Rule**     | Success screen just says "Done"         | Confetti, summary, clear next step                        |
