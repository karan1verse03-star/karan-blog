...
## 0. Tool Selection — Start Here

```
What do you need?
│
├── React component animation (enter/exit/hover/scroll)?
│   └── Framer Motion (always start here for React)
│
├── Complex branded animation from a designer?
│   └── Lottie (After Effects export)
│
├── Sequenced scroll-driven timeline?
│   └── GSAP + ScrollTrigger
│
├── Logo / icon path animation?
│   └── SVG (stroke-dashoffset or morph)
│
├── Interactive 3D effect?
│   └── CSS 3D Transforms (simple) or Three.js (complex)
│
└── Atmospheric background?
    └── tsParticles or Canvas API
```
...
| --------------------- | -------------------------- | --------------------------------- |
| **Best for**          | React components, UI state | Complex sequences, SVG, non-React |
| **Learning curve**    | Low                        | Medium-High                       |
| **Bundle size**       | ~30KB                      | ~20KB core + plugins              |
| **Layout animations** | ✅ Automatic               | ❌ Manual                         |
| **ScrollTrigger**     | `whileInView` (basic)      | ScrollTrigger plugin (powerful)   |
| **Spring physics**    | ✅ Built-in                | ❌ Custom                         |

> **Default to Framer Motion for React.** Only reach for GSAP when you need timeline control, SVG morphing, or scroll precision that Framer can't handle.
