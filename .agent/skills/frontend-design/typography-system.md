...
<!-- ADD AS SECTION 2.5 — between "Font Pairing Principles" and "Line Height" -->

## 2.5 Font Pairing — Concrete Examples

> These are starting points, not defaults. Use what fits the project context.

| Context            | Heading            | Body           | Mood                   |
| ------------------ | ------------------ | -------------- | ---------------------- |
| Modern SaaS        | Geist / Inter      | Inter          | Clean, technical       |
| Editorial / Blog   | Playfair Display   | Source Serif 4 | Classic, readable      |
| Bold startup       | Syne               | DM Sans        | Strong, contemporary   |
| Luxury / Premium   | Cormorant Garamond | Lato           | Refined, elevated      |
| Playful / Consumer | Nunito             | Nunito Sans    | Friendly, approachable |
| Brutalist          | Space Grotesk      | Space Grotesk  | Raw, opinionated       |

### Google Fonts Next.js Setup
...
## 5.5 Fluid Typography — Real Clamp Values
...
/* H1 — 36px to 56px */
font-size: clamp(2.25rem, 1.5rem + 3.75vw, 3.5rem);

/* Display / Hero — 48px to 80px */
font-size: clamp(3rem, 1.5rem + 7.5vw, 5rem);
```

### Tailwind v4 Type Scale Setup

```css
@theme {
  /* Fluid scale using clamp */
  --text-body: clamp(1rem, 0.925rem + 0.375vw, 1.125rem);
  --text-h3: clamp(1.25rem, 1.1rem + 0.75vw, 1.5rem);
  --text-h2: clamp(1.75rem, 1.375rem + 1.875vw, 2.25rem);
  --text-h1: clamp(2.25rem, 1.5rem + 3.75vw, 3.5rem);
  --text-hero: clamp(3rem, 1.5rem + 7.5vw, 5rem);
}
```
