...
## 11. Modern Pattern Decision Trees

### Hero Section

```
What's your primary goal for the hero?
│
├── Communicate value fast (SaaS, startup)
│   ├── → Massive typography + one strong CTA
│   └── → Headline + sub + CTA + social proof line
│
├── Create emotional impact (luxury, portfolio)
│   ├── → Full-bleed image/video + minimal overlay text
│   └── → Slow entrance animation, restrained copy
│
├── Show the product (app, tool)
│   ├── → Hero with product screenshot/demo on right
│   └── → Animate the product UI on scroll
│
└── Drive immediate action (e-commerce, event)
    ├── → CTA above the fold, urgency indicator
    └── → Price or offer visible without scrolling
```
...
│
├── 1–5 pages (portfolio, landing)
│   └── Minimal horizontal nav — no hamburger on desktop
│
├── 5–15 pages (startup, SaaS marketing)
│   └── Horizontal nav + "Get Started" CTA button always visible
│
├── 15+ pages (product, docs, app)
│   ├── → Sidebar navigation (persistent)
│   └── → Or mega-menu with categories
│
└── App / Dashboard
    └── Sidebar with icons + labels, collapsible
```
...
What's your data?
│
├── Visual content (products, projects, people)
│   └── Cards — show image, title, key action
│
├── Comparable items (pricing, features, specs)
│   └── Table — scannable, sortable
│
├── Sequential / linear content (posts, activity)
│   └── List — date, title, status
│
└── Dense data (dashboard, analytics)
